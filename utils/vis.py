...
def render_FFT(fft_img, num_bins_to_show, bin_size, num_azims, path,
                          resolution=781, noise_floor=None, save=True, fft = True, plot=False, norm=False):
...
def render_FFT_batch(fft_imgs, num_bins_to_show, bin_size, num_azims, path, timestamps, min_range_bin,
                     resolution=781, noise_floor=None, fft = True, save=True, no_override=False):
    ...
    for b in range(B):
        ...
        render_FFT(fft_img, num_bins_to_show, bin_size, num_azims, fpath,
                              resolution=resolution, noise_floor=noise_floor, save=save, fft=fft)
