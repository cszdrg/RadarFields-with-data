...
from radarfields.radar import avg_rays, integrate_rays_LUT, rcs_to_intensity, integrate_points_LUT, avg_rays_points
...
class Trainer(object):
    def __init__(self, args, model, split, criterion, optimizer, lr_scheduler,
        ...
        self.log_ptr = None
        self.gumbel_tau = 1
        self.gumbel_tau_min = 0.3
        self.gumbel_tau_decay =  0.999
        self.alpha = 0.0
        ...
        self.point_grid_path = self.img_path / "point_results"
        self.alpha_grid_path.mkdir(exist_ok=True)
        self.point_grid_path.mkdir(exist_ok=True)
        self.FFT_path = self.img_path / "FFT"
        self.FFT_path.mkdir(exist_ok=True)
        self.OCC_path = self.img_path / "OCC"
        self.OCC_path.mkdir(exist_ok=True)
    ...
    def save_figures(self, model_outputs, data_batch, points, loader):
        ...
        save_alpha_grid(points["xyz"], self.point_grid_path, timestamps)
        ...
        if self.args.render_fft:
            ...
            render_FFT_batch(data_batch["occ"], self.args.max_test_bin,
                             self.args.intrinsics_radar["bin_size_radar"],
                             self.args.intrinsics_radar["num_azimuths_radar"],
                             self.OCC_path, timestamps, self.args.min_range_bin,
                             resolution=self.args.figure_resolution,
                             noise_floor=self.args.noise_floor, fft = False)
    ...
    def test(self, loader):
        ...
        plot_trajectory(loader._data.poses_radar[1:,...], self.trajectories_path / "original_trajectory.png")
    ...
    def train_epoch(self, loader, epoch):
        ...
        for data in loader:
            ...
            points, azimuths, ranges_original, ranges, range_samples_idx = get_points(data, self.args, self.device, self.training, self.alpha) # model queries
            out = self.predict_waveform(data, points, ranges)
            loss = self.compute_loss(data, points, *out, azimuths, ranges_original, ranges, range_samples_idx)
            torch.cuda.synchronize()
            loss.backward()
            # if epoch % 10 == 0:
            #     bounds = loader._data.range_bounds
            #     bounds = (bounds[0], self.args.max_test_bin)
            #     img_path = self.img_path / str(epoch)
            #     img_path.mkdir(exist_ok=True)
            #     render_outputs_supersampled(self, data, points, bounds, img_path, data['timestamps'],
            #                         resolution=self.args.figure_resolution)
            # bounds = loader._data.range_bounds
            # bounds = (bounds[0], self.args.max_test_bin)
            # timestamps = data["timestamps"]
            # render_outputs_supersampled(self, data, points, bounds, self.img_path, timestamps,
            #                         resolution=self.args.figure_resolution)
            self.optimizer.step()
    ...
    def compute_loss(self, data, points, pred_fft, alpha_integrated, rd_integrated, alpha, azimuths = None, ranges_original = None, ranges = None, range_samples_idx = None):
        """
        ## Compute loss terms
        ...
        if self.training:
            ...
            num_bins = self.args.max_range_bin-self.args.min_range_bin+1
            fft = torch.gather(fft, 1, azimuths[...,None].expand((B,N,num_bins)))
            if self.args.reg_occ: occ = torch.gather(occ, 1, azimuths[...,None].expand((B,N,num_bins)))

            # If sampling all range bins per-ray, no need to filter the data anymore; return
            if not self.args.sample_all_ranges:

                # Else use only the sampled range bins
                fft = torch.gather(fft, 2, range_samples_idx-self.args.min_range_bin)
                if self.args.reg_occ: occ = torch.gather(occ, 2, range_samples_idx-self.args.min_range_bin)
        ...
        if self.args.reg_occ:
            ...
            elif self.args.occ_loss == "BCELoss":
                eps = 1e-6
                occ_penalty = self.criterion["occ"](
                    alpha_integrated_fp32.clamp(eps, 1 - eps),
                    occ
                )
            ...
            if self.args.bimodal and self.args.occ_loss != "BCELoss":
                mask_bim = occ > 0.01
    ...
    def predict_waveform(self, data, points, ranges, batch=True):
        '''
        ...
        # Query Radar Field
        out = self.model(xyz, directions, self.gumbel_tau, sin_epoch=self.sin_epoch)
        alpha = out["alpha"] # [B, N*S, R, 1]
        rd = out["rd"] # [B, N*S, R, 1]
        # rcs = alpha*rd # radar cross-section (RCS)
        rcs = alpha * torch.log10( rd /  ranges.unsqueeze(-1)**2)

        # 过滤地面以下的点
        if self.args.ground_filter:
            z_ground = -1
            z = points["xyz"][..., 2].unsqueeze(-1)
            alpha = torch.where(
                z >= z_ground,
                alpha,
            ...
            rd = torch.where(
                z >= z_ground,
                rd,
                torch.full_like(rd, 1e-6)
            )
            rcs = torch.where(
                z >= z_ground,
                rcs,
                torch.full_like(rcs, 1e-6)
            )
        ...
        if self.args.integrate_rays:
            ...
            alpha_integrated = avg_rays(alpha, S) if batch else avg_rays_points(alpha, S)
            rd_integrated = avg_rays(rd, S) if batch else avg_rays_points(rd, S)
            rcs = avg_rays(rcs, S) if batch else avg_rays_points(rcs, S)
