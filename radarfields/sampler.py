...
def init_sampler(data_batch, args, training=True, alpha = None):
    B = data_batch["bs"]
    ...
    sample_all_ranges = args.sample_all_ranges
    if not training: # Sample all azimuth-range bins during testing
        N = args.intrinsics_radar["num_azimuths_radar"]
        R = args.max_range_bin-args.min_range_bin+1
        sample_all_ranges = True

    if (args.max_range_bin-args.min_range_bin+1) ==args.num_range_samples: sample_all_ranges = True

    # Sample azimuth angles at which to query model (in terms of idx from 0 -> 400)
    # (sample all azimuths during test)
    azimuth_samples = get_azimuths(B, N, args.intrinsics_radar["num_azimuths_radar"], 
                                   args.device, all=not training)
    
    # Sample range bins at which to query model, and convert to radial distances in meters
    range_samples_idx = get_range_samples(B, N, R, data_batch, azimuth_samples, args,
                                        (args.min_range_bin, args.max_range_bin),
                                        device=args.device,
                                        alpha = alpha,
                                        all=sample_all_ranges) # [B, N, R]
    range_samples = range_to_world(range_samples_idx, args.intrinsics_radar["bin_size_radar"]) # [B, N, R]
    range_samples_expanded = range_samples.repeat_interleave(S, dim=1) # [B, N*S, R]

    return azimuth_samples, range_samples, range_samples_expanded, range_samples_idx

def get_points(data_batch, args, device, training=True, alpha = None):
    '''
    ...
    '''
    points = {}
    azimuths, ranges_original, ranges, range_samples_idx = init_sampler(data_batch, args, training, alpha)
    ...
    return points, azimuths, ranges_original, ranges, range_samples_idx

def get_range_samples(B, N, num_samples, data_batch, azimuth_samples, args, bounds, device, alpha = None, all=False):
    '''
    ...
    if args.importance_sampling:
        occ = data_batch["occ"] if "occ" in data_batch else None # [B, S, R]
        batch_idx = torch.arange(B).unsqueeze(1) # [B, 1]
        occ = occ[batch_idx, azimuth_samples].reshape(-1, num_bins) # [B*N, num_bins]
        uniform_w = torch.ones_like(occ) # [B*N, num_bins]
        weights = (1 - alpha) * uniform_w + alpha * occ
        ...
        return samples.reshape(B, N, num_samples)
