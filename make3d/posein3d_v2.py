...
# 核心功能：保存 GIF
# ==========================================
def save_trajectory_gif(pcd, poses, output_filename=".make3d/output.gif", fps=15):
    ...
    width, height, depth = 300, 300, 5  # 根据点云范围调节
    mesh_ground = o3d.geometry.TriangleMesh.create_box(width=width, height=height, depth=depth)
    ...
    mesh_ground.translate([-width/2, -height/2, -depth - 1])  # 放置到 y=0
...
if __name__ == "__main__":
    ...
    point_cloud = create_dummy_point_cloud(points, alpha, threshold=0.2)
    poses_radar = []
    ...
    save_trajectory_gif(point_cloud, poses_radar, "make3d/my_scan_result.gif")


# plt.figure()
# counts, bin_edges = np.histogram(a[:,2], bins=20)
